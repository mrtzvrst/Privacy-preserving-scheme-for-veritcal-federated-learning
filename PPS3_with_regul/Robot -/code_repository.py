# /home/mv19977/.local/lib/python3.6/site-packages/
...
Outlier_detection   
Log_Reg_initial	        logistic regression with arbitrary initials
Log_Reg                 
NeuralNet_Relu          (VFL)
NeuralNet_Tanh          (VFL)
Loss_function 
...
train_VFL_NN
test_VFL_NN
train_LR                Check the lagrangian for the biases as well
test_LR
PPS1
PPS2
PPS3
...
class Log_Reg_initial(nn.Module):
    def __init__(self, input_dim, output_dim, W, b):
        super(Log_Reg_initial, self).__init__()
        self.linear = torch.nn.Linear(input_dim, output_dim)
        self.linear.weight = torch.nn.Parameter(W)
        self.linear.bias = torch.nn.Parameter(b)
...
"""Define the Logistic Regression model (softmax is automatically defined in the loss function)"""
class Log_Reg_PPS(nn.Module):
    def __init__(self, input_dim, output_dim):
        super(Log_Reg_PPS, self).__init__()
        # self.linear = torch.nn.Linear(input_dim, output_dim)
        self.linear = torch.nn.Linear(input_dim, output_dim, bias = False)
    def forward(self, x):
        #CrossEntropyLoss automatically applies the softmax function on the output of the network
...
class NeuralNet_Relu(nn.Module):
    def __init__(self, da, dp, output_dim):
        ...
        self.layers_act = nn.Sequential(
          # First upsampling
          nn.Linear(da, min(output_dim+2, da), bias=True),
          # nn.BatchNorm1d(8),
          nn.ReLU(),
          # Second upsampling
          nn.Linear(min(output_dim+2, da), min(output_dim+1, da), bias=True),
          # nn.BatchNorm1d(8),
          nn.ReLU(),
          # Third upsampling
          nn.Linear(min(output_dim+1, da), output_dim, bias=True),
        )
        self.layers_pas = nn.Sequential(
          # First upsampling
          nn.Linear(dp, min(output_dim+2, dp), bias=True),
          # nn.BatchNorm1d(8),
          nn.ReLU(),
          # Second upsampling
          nn.Linear(min(output_dim+2, dp), min(output_dim+1, dp), bias=True),
          # nn.BatchNorm1d(8),
          nn.ReLU(),
          # Third upsampling
          nn.Linear(min(output_dim+1, dp), output_dim, bias=True),
        )
...
def Diff_Bound(Weights, X):
    ...
    while k0 < Num_of_Features:
        ...
        while i < t0:
            ...
            Diff[0,i] += 4/(i+1)*np.trace(np.matmul(np.matmul(Aplus, A),KHalf))
                
            i+=1
        k0+=1
    
    return Diff/Num_of_Features
            ...
            while ind< Num_of_Predictions:
                ...
                if 0<= i < Num_of_classes-1:
                    ...
                    if Version == 'Extended':
                        ...
                        m2 = np.matmul(m1, np.ones((i+1,1)))/2.
                        X_pas = (m0 + m2).flatten()
...
"""Train"""
def train_LR(model, epochs, optimizer, CEF_loss, Lambda, batch_size, X_train, Y_train, X_valid, Y_valid, seed):
    model.train()
    num_mini_batches = int(X_train.shape[0]/batch_size)
...
"""Train"""
def train_LR_PPS(model, epochs, optimizer, CEF_loss, L1, L2, batch_size, X_train, Y_train, X_valid, Y_valid, seed):
    model.train()
    num_mini_batches = int(X_train.shape[0]/batch_size)
    ...
    for i in range(epochs+1):
        ...
        for MB_X, MB_Y in minibatches:
            ...
            optimizer.zero_grad()
            Y_out = model.forward(MB_X)
            loss = CEF_loss(Y_out,MB_Y) + L1* torch.sum(model.linear.weight**2) + L2*torch.sum(model.linear.bias**2)
            loss.backward()
            train_loss += loss.item() 
            optimizer.step()
...
"""Test"""
def test_LR(Final_model, X_test, Y_test, X_train, Y_train):
    Final_model.eval()
...
def PPS1(k0_stp, i_stp, Num_of_Predictions, filename, STR, Truncate, Version, epochs, batch_size, accuracy, Model_NT_name, seed):
    ...
    while k0 < Num_of_Features:
        ...
        while i < t0:
            ...
            missing_features = np.mod(np.arange(k0,i+1+k0), Num_of_Features)
            fn = 'Model_PPS1_'+str(k0)+'_'+str(i)+'.pckl'
            if exists(fn):
                f = open(fn, 'rb')
                model_data = pickle.load(f)
                f.close()
                ...
                while acc<accuracy:
                    ...
                    x_ts, x_vl, y_ts, y_vl = train_test_split(x_ts, y_ts, test_size = 0.5, random_state=seed)
                    ...
                    Final_model1 = train_LR(model1, epochs, optimizer1, CEF_loss1, Lambda, batch_size, x_tr, y_tr, x_vl, y_vl, 'no', seed)
                    acc = test_LR(Final_model1, x_ts, y_ts, x_tr, y_tr)
...
def PPS2(k0_stp, i_stp, Num_of_Predictions, filename, STR, Truncate, Version, epochs, batch_size, accuracy, Model_NT_name, seed):
    ...
    while k0 < Num_of_Features:
        ...
        while i < t0:
            ...
            missing_features = np.mod(np.arange(k0,i+1+k0), Num_of_Features)
            fn = 'Model_PPS2_'+str(k0)+'_'+str(i)+'.pckl'
            if exists(fn):
                f = open(fn, 'rb')
                model_data = pickle.load(f)
                f.close()
                ...
                X, Y = model[1], model[2]
                M1 = My_Optimization(missing_features, X).Matric_obtain()
...
def PPS3(k0_stp, i_stp, Num_of_Predictions, filename, STR, Truncate, Version, epochs, batch_size, accuracy, Model_NT_name, seed):
    ...
    Num_of_samples = X1.shape[0]
    Num_of_Features = X1.shape[1]
    Lambda1, Lambda2, learning_rate, Num_of_classes = model_data[9], model_data[10], model_data[11], len(list(model_data[0].named_parameters())[1][1])
    ...
    k0 = 0
    while k0 < Num_of_Features:
        i = 0 # length of features should be more than one
        while i < t0:
            
            missing_features = np.mod(np.arange(k0,i+1+k0), Num_of_Features)
            fn = 'Model_PPS3_'+str(k0)+'_'+str(i)+'.pckl'
            if exists(fn):
                f = open(fn, 'rb')
                model_data = pickle.load(f)
                f.close()
                ...
                X[:,missing_features] = 1.-X[:,missing_features]
                ...
                while acc<accuracy:
                    ...
                    Final_model1 = train_LR_PPS(model1, epochs, optimizer1, CEF_loss1, Lambda1, Lambda2, batch_size, x_tr, y_tr, x_vl, y_vl, seed)
                    acc = test_LR(Final_model1, x_ts, y_ts, x_tr, y_tr)
                    
                PARAM = (Final_model1, X, Y, x_tr.detach().numpy(), y_tr.detach().numpy(), x_ts.detach().numpy(), y_ts.detach().numpy(), x_vl.detach().numpy(), y_vl.detach().numpy(), Lambda1, Lambda2, learning_rate)
                Weights = np.array(list(PARAM[0].named_parameters())[0][1].detach().cpu(), dtype = np.float64)
                Biases = np.array(list(PARAM[0].named_parameters())[1][1].detach().cpu(), dtype = np.float64)
