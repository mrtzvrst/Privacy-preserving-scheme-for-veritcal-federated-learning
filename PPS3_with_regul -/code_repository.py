...
class NeuralNet_Relu(nn.Module):
    def __init__(self, da, dp, output_dim):
        ...
        self.layers_act = nn.Sequential(
          # First upsampling
          nn.Linear(da, output_dim-1, bias=True),
          # nn.BatchNorm1d(8),
          nn.ReLU(),
          # Second upsampling
          nn.Linear(output_dim-1, output_dim-1, bias=True),
          # nn.BatchNorm1d(8),
          nn.ReLU(),
          # Third upsampling
        ...
        self.layers_pas = nn.Sequential(
          # First upsampling
          nn.Linear(dp, output_dim-1, bias=True),
          # nn.BatchNorm1d(8),
          nn.ReLU(),
          # Second upsampling
          nn.Linear(output_dim-1, output_dim-1, bias=True),
...
"""Train"""
def train_LR(model, epochs, optimizer, CEF_loss, Lambda, batch_size, X_train, Y_train, X_valid, Y_valid, Early_stop, seed):
    model.train()
    num_mini_batches = int(X_train.shape[0]/batch_size)
    ...
    for i in range(epochs+1):
        ...
        for MB_X, MB_Y in minibatches:
            ...
            # loss = CEF_loss(Y_out,MB_Y) + Lambda* sum([torch.sum(p**2) for p in model.parameters()])
            loss = CEF_loss(Y_out,MB_Y) + Lambda* torch.sum(model.linear.weight**2) + 10**-10*torch.sum(model.linear.bias**2)
            loss.backward()
            train_loss += loss.item() 
            optimizer.step()
    ...
    return Final_model if Early_stop=='yes' else model
